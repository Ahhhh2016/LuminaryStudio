...
void Flame::initialize() {
    ...
    mStartTexture.loadTexture("./resources/flame.bmp");

    glUseProgram(mRenderShader);
    // mRenderShader->use();
    // mRenderShader->sextInt("water", 0);
    glUniform1i(glGetUniformLocation(mRenderShader, "flameSpark"), 0);
    glUniform1i(glGetUniformLocation(mRenderShader, "flameStart"), 1);
//    mRenderShader->use();
