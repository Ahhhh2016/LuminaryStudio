...
void MainWindow::initialize() {
    ...
    aspectRatioWidget->setAspectWidget(realtime, 1080.f/1920.f);
    ...
    // filter2->setText(QStringLiteral("Sharpen Filter"));
    ...
    // // Extra Credit:
    // ec1 = new QCheckBox();
    // ec1->setText(QStringLiteral("Adaptive Detail"));
    // ec1->setChecked(false);

    // ec2 = new QCheckBox();
    // ec2->setText(QStringLiteral("Texture Mapping"));
    // ec2->setChecked(false);

    // ec3 = new QCheckBox();
    // ec3->setText(QStringLiteral("Grayscale Filter"));
    // ec3->setChecked(false);

    // ec4 = new QCheckBox();
    // ec4->setText(QStringLiteral("Blur Filter"));
...
void MainWindow::onPerPixelFilter() {
    settings.invert_filter = !settings.invert_filter;
    realtime->settingsChanged();
}

void MainWindow::onKernelBasedFilter() {
    settings.sharpen_filter = !settings.sharpen_filter;
    realtime->settingsChanged();
}
...
void MainWindow::onExtraCredit1() {
    settings.adaptive_detail = !settings.adaptive_detail;
    realtime->settingsChanged();
}

void MainWindow::onExtraCredit2() {
    settings.use_texture = !settings.use_texture;
    realtime->settingsChanged();
}

void MainWindow::onExtraCredit3() {
    settings.grayscale_filter = !settings.grayscale_filter;
    realtime->settingsChanged();
}

void MainWindow::onExtraCredit4() {
    settings.blur_filter = !settings.blur_filter;
    realtime->settingsChanged();
}
